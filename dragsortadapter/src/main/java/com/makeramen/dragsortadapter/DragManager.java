    ...
    public boolean onDrag(View v, DragEvent event) {
        ...
        switch (event.getAction()) {
                    ...
                    if (scheduleNextMove)
                        animator.isRunning(new RecyclerView.ItemAnimator.ItemAnimatorFinishedListener() {
                            ...
                            public void onAnimationsFinished() {
                                ...
                                if (child != null) {
                                    ...
                                    if (adapter.move(fromPosition, toPosition)) {
                                        ...
                                        if (fromPosition == 0 || toPosition == 0) {
                                            // fix for weird scrolling when
                                            // animating first item
                                ...
                                // reset so we know to schedule listener again next
                                // time
                                clearNextMove();
                            }
                        });
            ...
            case DragEvent.ACTION_DRAG_ENDED:
                ...
                // queue up the show animation until after all move animations are
                // finished
                recyclerView.getItemAnimator().isRunning(new RecyclerView.ItemAnimator.ItemAnimatorFinishedListener() {
                    ...
                    public void onAnimationsFinished() {
                        ...
                        if (vh != null && vh.getAdapterPosition() != position) {
                            // if positions don't match, there's still an
                            // outstanding move animation
                            // so we try to reschedule the notifyItemChanged until
                            // after that
                            recyclerView.post(new Runnable() {
                                @Override
                                public void run() {
